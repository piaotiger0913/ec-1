...
static void emlrt_marshallIn(const emlrtStack *sp, const mxArray *rec,
                             ...
                             const char_T *identifier, emxArray_real_T *y)
{
  ...
  b_emlrt_marshallIn(sp, emlrtAlias(rec), &thisId, y);
...
void cal_corr_api(const mxArray *const prhs[2], const mxArray **plhs)
{
  ...
  emxInit_real_T(&st, &rec, 2, true);
  emxInit_real_T(&st, &play, 2, true);
  /* Marshall function inputs */
  rec->canFreeData = false;
  emlrt_marshallIn(&st, emlrtAlias(prhs[0]), "rec", rec);
  play->canFreeData = false;
  emlrt_marshallIn(&st, emlrtAlias(prhs[1]), "play", play);
...
void cal_corr_atexit(void)
{
  ...
  cal_corr_xil_terminate();
  cal_corr_xil_shutdown();
...
/*
 * File trailer for _coder_cal_corr_api.c
 *
 * [EOF]
 */
